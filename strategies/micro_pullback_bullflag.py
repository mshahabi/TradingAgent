...
def backtest(
    ...
    for date in selected_stocks.keys():
        ...
        for ticker in selected_stocks[date]:
            ticker_event.clear()
            histData(app, reqID, usTechStk(ticker), date + " 22:05:00 US/Eastern", '10 D', '5 mins')
            ticker_event.wait()
            ...
            df['DateTime'] = df['Date'].apply(
                lambda date: datetime.strptime(date.split(' ')[0] + ' ' + date.split(' ')[1], "%Y%m%d %H:%M:%S")
            )
            ...
            df['StrongMomentum'] = (df['Green']) & (df['Close'] > df['Close'].shift(1) * 1.002) & (df['Close'].shift(1) > df['Close'].shift(2) * 1.002)
            df['ATR'] = df['High'].rolling(14).max() - df['Low'].rolling(14).min()
            df['Momentum'] = df['StrongMomentum'] & ((df['High'] - df['Low']) > 0.5 * df['ATR'])
            ...
            df['MicroPullback'] = (
                df['Momentum'].shift(1) & df['PullbackAboveVWAP'] &
                df['NotExtended'] & df['VWAP_Reclaim'] & df['VolumeSpike']
            )

            # === Bull Flag Setup ===
            df['PriceChange'] = df['Close'] / df['Close'].shift(5)
            df['FlagPole'] = df['PriceChange'] > 1.03  # 3% in 5 candles
            df['Consolidation'] =((df['High'].rolling(3).max() - df['Low'].rolling(3).min()) < 0.015 * df['Close']) &  (df['Close'] < df['Close'].shift(1))
            df['Breakout'] = (df['Close'] > df['High'].rolling(5).max().shift(1)) & (df['RelativeVolume'] > 1.5)
            df['BullFlag'] = df['FlagPole'].shift(3) & df['Consolidation'] & df['Breakout']
            ...
            for i in range(len(df)):
                ...
                if not in_position:
                    if df.iloc[i]["MicroPullback"] or df.iloc[i]["BullFlag"]:
                        ...
                        strategy = "MicroPullback" if df.iloc[i]["MicroPullback"] else "BullFlag"
                        transactions.append((buy_date, ticker, f'BUY_{strategy}', entry_price))
                        in_position = True
