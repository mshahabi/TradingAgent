...
def compute_micro_pullback_ema_strategy(
    ...
    # === Crossovers ===
    df['EMA4_cross_EMA7'] = (df['EMA4'].shift(2) <= df['EMA7'].shift(2)) & (df['EMA4'].shift(1) > df['EMA7'].shift(1))
    ...
    df['VolumeSpike'] = (df['Volume'] > df['AvgVolume'] * volume_spike_factor) & (df['Volume'] > 15000)
    df['EMA4_pct_change'] = df['EMA4'].pct_change()

    # === EMA Slope ===
    df['EMA15_Slope'] = df['EMA15'].rolling(window=slope_window).apply(compute_slope, raw=False)
    ...
    return df[[
        'DateTime', 'EMA4', 'EMA7', 'EMA15', 'EMA4_pct_change',
        'EMA15_Slope', 'ChoppinessIndex', 'ADX', 'BreakoutConfirm',
        'StrongClose', 'EMABuySignal'
    ]]