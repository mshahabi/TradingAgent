...
def backtest(
    ...
    for date in selected_stocks.keys():
        ...
        for ticker in selected_stocks[date]:
            ...
            df['VolumeSpike'] = (df['RelativeVolume'] > 5) & (df['Volume'] > 10000)
    ...
    df['Green'].shift(5) & df['Green'].shift(4) & df['Green'].shift(3)
            ...
            for i in range(len(df)):
                ...
                if not in_position:
                    ...
                    buy_date = row["Date"]
                    transactions.append((buy_date, ticker, 'BUY', entry_price, entry_type))
                    in_position = True
                    ...
                    if close_price >= target_price or close_price <= stop_loss:
                        ...
                        sell_date = row["Date"]
                        transactions.append((sell_date, ticker, 'SELL', exit_price, entry_type))
